...
modified code for ns-3.33/src/aodv/model/aodv-routing-protocol.cc
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    /** 
      ENSC 894 Communication Networks
      Security Analysis of AODV Protocol
      code start
    **/
    .AddAttribute ("EnableGrayAttack", "Indicates whether a Grayhole Attack is enabled or not.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&RoutingProtocol::SetGrayAttackEnable,
                                        &RoutingProtocol::GetGrayAttackEnable),
                                        MakeBooleanChecker ())
    .AddAttribute ("EnableBlackAttack", "Indicates whether a Black Attack is enabled or not.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&RoutingProtocol::SetBlackAttackEnable,
                                        &RoutingProtocol::GetBlackAttackEnable),
                                        MakeBooleanChecker ())
    .AddAttribute("EnableWrmAttack", "Indicates whether a Wormhole Attack is enabled or not.",
                   BooleanValue(false),
      ...
      if (lcb.IsNull () == false)
        {
          ...
          /**
              ENSC 894 Communication Networks
              Security Analysis of AODV Protocol
              code for wormhole start
          **/  
          if(EnableWrmAttack)
          		  {
          			  if(dst==FirstEndOfWormTunnel || dst==SecondEndOfWormTunnel)
          			  {
          				  std::cout <<"Wromhole Enabled, tunnel triggered\n";
          				  iif=1;
          			  }
          		  }
          /**
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
   ...
   * Added for 
     ...
     code start
  **/    
  if(EnableBlackAttack || EnableGrayAttack || EnableWrmAttack)
    {
...
if(EnableWrmAttack) 
    ...
    reference: https://github.com/rahulhada/ns3-wormhole-attack/blob/master/aodv-routing-protocol-cc.patch
    */
	  if(sender==FirstEndOfWormTunnel && receiver==SecondEndWifiWormTunnel)
  		{                     
 			std::cout<<"Received by Second Wifi Wrm Tunnel"<<std::endl;
     ...
     */
	  if(sender==SecondEndOfWormTunnel && receiver==FirstEndWifiWormTunnel)
  		{
 			std::cout<<"Received by First Wifi Wrm Tunnel"<<std::endl;
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if (!m_routingTable.LookupRoute (origin, toOrigin))
     ...
     */       
     if(EnableWrmAttack && (src==FirstEndOfWormTunnel))
      	  {
     		std::cout<<"ENTER IN THE ATTACK WRM2";
    ...
    */
      	  ...
      	  {
     		std::cout<<"ENTER IN THE ATTACK WRM1";
  ...
  /**
              ENSC 894 Communication Networks
              Security Analysis of AODV Protocol
              code for blackhole start
  **/    
  // if blackhole attack enabled, the false routing entry will be added to the routing table
  if (EnableBlackAttack || m_routingTable.LookupRoute (dst, toDst))
    {
      /*
       ...
       */
      if (EnableBlackAttack || ((rreqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (rreqHeader.GetDstSeqno ()) >= 0)) && toDst.GetValidSeqNo () ) )
        {
          if (EnableBlackAttack || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
            {
              m_routingTable.LookupRoute (origin, toOrigin);
                ...
                * so that the route will be established through malicious node.
                reference: http://mohittahiliani.blogspot.com/2014/12/ns-3-blackhole-attack-simulation-in-ns-3.html
              */
              if(EnableBlackAttack)
                  {
		     ...
		     std::cout << "Malicious node itself sends the RREP message\n";
		     return;
                  }
  /**
              ENSC 894 Communication Networks
              Security Analysis of AODV Protocol
              code for blackhole end
  **/                  
...
RoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
  ...
  if (!m_routingTable.LookupRoute (rrepHeader.GetDst (), toNeighbor))
     ...
     */
      ...
      {
    	  std::cout<<"RREP Helper Contains First End P2P interface Address";
     ...
     */
      ...
      {
    	  std::cout<<"RREP Helper Contains Second End P2P interface Address";
